...
export const DefaultLeaf = ({ name, tags, ...rest }) => (
  <LeafStyle data-tags={tags} {...rest}>
    {name}
  </LeafStyle>
);
...
export const DefaultHead = ({ name, tags, depth, isExpanded = true, isSelected, isComponent }) => (
  <LeafStyle data-tags={tags} isSelected={isSelected} depth={depth}>
    <span>
      {isExpanded ? '-' : '+'}
      {isComponent ? '!' : ''}
    </span>
...
export const DefaultLink = ({ id, prefix, tags, children, ...rest }) => (
  <A
    href={`#!${prefix}${id}`}
    {...rest}
    onClick={e => e.preventDefault() || rest.onClick(e)}
    data-tags={tags}
  >
    {children}
  </A>
